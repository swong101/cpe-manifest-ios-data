    ...
    override init?(indexer: XMLIndexer) throws {
        ...
        if let regionCode = CPEXMLSuite.Settings.countryCode {
            if indexer.hasElement(Elements.Region) {
                let supportedRegions: [String] = try indexer[Elements.Region].all.map({ try $0[Elements.Country].value() })
                ...
                let unsupportedRegions: [String] = try indexer[Elements.ExcludedRegion].all.map({ try $0[Elements.Country].value() })
