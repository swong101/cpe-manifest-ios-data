...
open class Presentation {
    ...
    init(indexer: XMLIndexer) throws {
        ...
        for indexer in indexer[Elements.TrackMetadata].all {
            // VideoTrackReference
            let videoIDs: [String] = try indexer[Elements.VideoTrackReference].all.map({ try $0[Elements.VideoTrackID].value() })
            if videoIDs.count > 0 {
            ...
            // AudioTrackReference
            let audioIDs: [String] = try indexer[Elements.AudioTrackReference].all.map({ try $0[Elements.AudioTrackID].value() })
            if audioIDs.count > 0 {
