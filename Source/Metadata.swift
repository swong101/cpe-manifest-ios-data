...
open class LocalizedInfo {
    ...
    init(indexer: XMLIndexer) throws {
        ...
        if indexer.hasElement(Elements.ArtReference) {
            artReferences = try indexer[Elements.ArtReference].all.map({ try ArtReference(indexer: $0) })
        }
...
open class Metadata {
    ...
    init(indexer: XMLIndexer) throws {
        ...
        localizedInfos = try metadataIndexer[Elements.LocalizedInfo].all.map({ try LocalizedInfo(indexer: $0) })
        ...
        if metadataIndexer.hasElement(Elements.People) {
            people = try metadataIndexer[Elements.People].all.map({ try Person(indexer: $0) }).sorted(by: { (person1, person2) -> Bool in
